...
if (!empty($lastapp)) {
    $where = !empty($where) && strlen($where) > 8 
        ? "$where $search_condition $lastapp" 
        : "$where $lastapp";
}
...
if (!empty($duration)) {
    $where = !empty($where) && strlen($where ?? '') > 8 
        ? "$where $search_condition $duration" 
        : "$where $duration";
}
...
if (!empty($billsec)) {
    $where = !empty($where) && strlen($where ?? '') > 8 
        ? "$where $search_condition $billsec" 
        : "$where $billsec";
}
...
if (isset($_REQUEST['need_csv']) && $_REQUEST['need_csv'] == 'true') {
    ...
    if (!is_file(Config::get('system.tmp_dir') . '/' . $csv_fname)) {
        ...
        while ($row = $sth->fetch(PDO::FETCH_ASSOC)) {
            ...
            $csv_line[15]	= $row['peeraccount'] ?? '';
            $csv_line[16]	= $row['uniqueid'];
            $csv_line[17]	= $row['userfield'];
            $csv_line[18]	= $row['linkedid'] ?? '';
            $csv_line[19]	= $row['sequence'] ?? '';
            $data = '';
            if ($use_callrates === true) {
            ...
            for ($i = 0; $i < count($csv_line); $i++) {
                $csv_line[$i] = str_replace(["\n", "\r"], '', $csv_line[$i]);
                /* If the string contains a comma, enclose it in double-quotes. */
                if (str_contains($csv_line[$i], $csv_delim)) { 	// ,
...
if (isset($_REQUEST['need_asr_report']) && $_REQUEST['need_asr_report'] == 'true') {
    ...
    try {
        ...
        while ($row = $sth->fetch(PDO::FETCH_NUM)) {
            if ($asr_cur_key != '' && $row[0] != $asr_cur_key) {
                echo '<tr  class="record">';
                echo "<td class=\"end_col\">$asr_cur_key</td><td class=\"chart_data\">", intval(($asr_answered_calls / $asr_total_calls) * 100), '</td><td class="chart_data">', intval($asr_bill_secs / ($asr_answered_calls ?: 1)), "</td><td class=\"chart_data\">$asr_total_calls</td><td class=\"chart_data\">$asr_answered_calls</td><td class=\"chart_data\">$asr_bill_secs</td>";
                echo '</tr>';
                $asr_answered_calls = 0;
                $asr_total_calls = 0;
                $asr_bill_secs = 0;
    ...
    if ($asr_cur_key != '') {
        echo '<tr class="record">';
        echo "<td class=\"end_col\">$asr_cur_key</td><td class=\"chart_data\">", ($asr_total_calls ? intval(($asr_answered_calls / $asr_total_calls) * 100) : 0), '</td><td class="chart_data">', intval($asr_bill_secs / ($asr_answered_calls ?: 1)), "</td><td class=\"chart_data\">$asr_total_calls</td><td class=\"chart_data\">$asr_answered_calls</td><td class=\"chart_data\">$asr_bill_secs</td>";
        echo '</tr>';
    }

    echo '<tr>';
    echo "<th class=\"chart_data\">Всего</th><th class=\"chart_data\">", ($all_asr_total_calls ? intval(($all_asr_answered_calls / $all_asr_total_calls) * 100) : 0), '</th><th class="chart_data">', intval($all_asr_bill_secs / ($all_asr_answered_calls ?: 1)), "</th><th class=\"chart_data\">$all_asr_total_calls</th><th class=\"chart_data\">$all_asr_answered_calls</th><th class=\"chart_data\">$all_asr_bill_secs</th>";
    echo '</tr>';
    echo '</table>';
}
