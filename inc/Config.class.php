...
class Config
{
     ...
     *
     * @return bool TRUE Если удалось установить новый путь к конфиг-файлу
     */
        ...
        while ($part = array_shift($segs)) {
            ...
            if (count($segs) === 0) {
                foreach (self::$cache as $cacheLocalKey => $cacheValue) {
                    if (str_starts_with((string) $cacheLocalKey, $cacheKey)) {
     ...
     * @param $default Возвращаемое значение, если такой параметр отсутствует
     *
     * @return bool TRUE Если такой параметр существует
     */
