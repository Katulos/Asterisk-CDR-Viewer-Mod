...
if (isset($_POST['delete_record'])) {
    if ((string) $cdr_user_name !== '' || Config::get('display.main.rec_delete') == 0) {
        header('HTTP/1.1 403 Forbidden');
        exit;
    }
    ...
    $path = Config::get('system.monitor_dir') . '/' . base64_decode((string) $data->path);
    if (file_exists($path) && is_file($path)) {
        if (@unlink($path)) {
            // Очистка поля с именем файла записи звонка
            if (($dbh = dbConnect(false)) instanceof \PDO) {
                $sth = $dbh->prepare('
            ...
            echo json_encode([
                'success' => $res,
                'message' => $res ? 'Успешно удалено' : 'Файл записи удален, но не удалось удалить имя файла из базы',
            ]);
        } else {
            echo json_encode([
                'success' => false,
...
if (isset($_POST['edit_userfield'])) {
    if ((string) $cdr_user_name !== '' || Config::get('display.main.userfield_edit') == 0) {
        header('HTTP/1.1 403 Forbidden');
        exit;
    }
    ...
    if (($dbh = dbConnect(false)) instanceof \PDO) {
        ...
        $sth = $sth->execute([
            'id' => $data->id,
            'text' => htmlspecialchars((string) $data->text),
        ]);
        if ($sth) {
            $res = true;
        }
...
if (isset($_POST['delete_entry'])) {
    if ((string) $cdr_user_name !== '' || Config::get('display.main.entry_delete') == 0) {
        header('HTTP/1.1 403 Forbidden');
        exit;
    }
