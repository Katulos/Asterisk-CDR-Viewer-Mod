...
// Recorded file
function formatFiles(array $row, array $file_params): void
{
    ...
    // Архив
    elseif ($file_params['type'] == 'archive') {
        $tmp['result'] = $tpl['download'];
    }
    // Факс
    elseif ($file_params['type'] == 'fax') {
        $tmp['result'] = $tpl['download'];
    }
...
function getFileParams(array $row): array
{
    // uniq_name.mp3
    $recorded_file = '';
    $tmp['result'] = [
    ...
    } elseif ($mycalldate_ymd < $mydate && Config::get('system.storage_format') === 2) {
        $rec['filename'] = "$mycalldate_y/$mycalldate_m/$mycalldate_d/$recorded_file";
    } elseif (Config::get('system.storage_format') === 3) {
        $rec['filename'] = "$mycalldate_y/$mycalldate_ym/$mycalldate_ymd/$recorded_file";
    } elseif (Config::get('system.storage_format') === 4) {
        $rec['filename'] = "$mycalldate_y/$mycalldate_m/$mycalldate_d/$recorded_file";
    } elseif (Config::get('system.storage_format') === 5) {
        $rec['filename'] = $recorded_file;
    } elseif (Config::get('system.storage_format') === 6) {
...
function formatCallDate($calldate, $uniqueid): void
{
...
function formatChannel($channel): void
{
    $chan['short'] = preg_replace('#(.*)\/[^\/]+$#', '$1', (string) $channel);
    $chan['full'] = preg_replace('#(.*)-[^-]+$#', '$1', (string) $channel);
...
function formatClid($clid): void
{
    $clid_only = explode(' <', (string) $clid, 2);
...
function formatSrc($src, $clid): void
{
    if (empty($src)) {
        echo '<td class="record_col">Неизвестно</td>' . PHP_EOL;
    } else {
        $src = htmlspecialchars((string) $src);
        $clid = htmlspecialchars((string) $clid);
        $src_show = $src;
        $clipboard = 'data-clipboard data-clipboard-text="' . $clid . '"';
        if (is_numeric($src) && strlen($src) >= Config::get('display.lookup.num_length') && (string) Config::get('display.lookup.url') !== '') {
...
function formatApp($app, $lastdata): void
{
...
function formatDst($dst, $dcontext): void
{
    $dst_show = $dst;
    $clipboard = 'data-clipboard data-clipboard-text="' . $dcontext . '"';
    if (is_numeric($dst) && strlen($dst) >= Config::get('display.lookup.num_length') && (string) Config::get('display.lookup.url') !== '') {
...
function formatDisposition($disposition, $amaflags): void
{
...
function formatDuration($duration, $billsec): void
{
...
function formatDurWait($duration, $billsec): void
{
...
function formatBillSec($duration, $billsec): void
{
    $duration = sprintf('%02d', intval($duration / 60)) . ':' . sprintf('%02d', intval($duration % 60));
    $billduration = sprintf('%02d', intval($billsec / 60)) . ':' . sprintf('%02d', intval($billsec % 60));
...
function formatUserField($userfield): void
{
    echo '<td class="record_col userfield">' . $userfield . '</td>' . PHP_EOL;
}

function formatAccountCode($accountcode): void
{
...
// title: title to show
function formatMoney($number, $cents = 2, $title = ''): void
{
...
*/
function callrates($dst, $duration, $file): array
{
    ...
    if (!array_key_exists($file, $callrate_cache)) {
        ...
        while (($fr_data = fgetcsv($fr, 1000, ',')) !== false) {
            if ($fr_data[0] !== null) {
                ...
                $callrate_cache[$file][$fr_data[0]] = [$fr_data[1], $fr_data[2], $fr_data[3], $fr_data[4]];
            }
        }
    ...
    for ($i = strlen((string) $dst); $i > 0; $i--) {
        if (array_key_exists(substr((string) $dst, 0, $i), $callrate_cache[$file])) {
            $call_rate = 0;
            if ($callrate_cache[$file][substr((string) $dst, 0, $i)][2] == 's') {
                // per second
                if ($duration >= Config::get('callrate.free_interval')) {
                    $call_rate = $duration * ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 60);
                }
            } elseif ($callrate_cache[$file][substr((string) $dst, 0, $i)][2] == 'c') {
                // per call
                if ($duration >= Config::get('callrate.free_interval')) {
                    $call_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0];
                }
            } elseif ($callrate_cache[$file][substr((string) $dst, 0, $i)][2] == '1m+s') {
                // 1 minute + per second
                if ($duration < Config::get('callrate.free_interval')) {
                } elseif ($duration < 60) {
                    $call_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0];
                    ...
                    // указан доп тариф
                    if (isset($callrate_cache[$file][substr((string) $dst, 0, $i)][3]) && $callrate_cache[$file][substr((string) $dst, 0, $i)][3] && $callrate_cache[$file][substr((string) $dst, 0, $i)][3] > 0) {
                        $ext_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][3];
                    } else {
                        $ext_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 60;
                    }

                    $call_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0] + (($duration - 60) * ($ext_rate));
                }
            } elseif ($callrate_cache[$file][substr((string) $dst, 0, $i)][2] == '30s+s') {
                // 30 second + per second
                if ($duration > 0 && $duration <= 30) {
                    $call_rate = ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 2);
                } elseif ($duration > 30 && $duration < 60) {
                    $call_rate = ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 2) + (($duration - 30) * ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 60));
                } else {
                    $call_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0] + (($duration - 60) * ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 60));
                }
            } elseif ($callrate_cache[$file][substr((string) $dst, 0, $i)][2] == '30s+6s') {
                // 30 second + 6 second
                if ($duration > 0 && $duration <= 30) {
                    $call_rate = ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 2);
                } else {
                    $call_rate = ceil($duration / 6) * ($callrate_cache[$file][substr((string) $dst, 0, $i)][0] / 10);
                }
            } else {
                ...
                if ($duration >= Config::get('callrate.free_interval')) {
                    ...
                    // указан доп тариф
                    if (isset($callrate_cache[$file][substr((string) $dst, 0, $i)][3]) && $callrate_cache[$file][substr((string) $dst, 0, $i)][3] && $callrate_cache[$file][substr((string) $dst, 0, $i)][3] > 0) {
                        $call_rate = $callrate_cache[$file][substr((string) $dst, 0, $i)][0] + (($call_rate - 1) * $callrate_cache[$file][substr((string) $dst, 0, $i)][3]);
                    } else {
                        $call_rate *= $callrate_cache[$file][substr((string) $dst, 0, $i)][0];
                    }
                }
            ...
            return [substr((string) $dst, 0, $i), $callrate_cache[$file][substr((string) $dst, 0, $i)][0], $callrate_cache[$file][substr((string) $dst, 0, $i)][1], $callrate_cache[$file][substr((string) $dst, 0, $i)][2], $call_rate];
        }
    }

    return [0, 0, 'Неизвестно', 'Неизвестно', 0];
}
...
*/
function dbConnect($errors = true): ?\PDO
{
    $dbh = null;
    try {
        $dbh = new PDO(
